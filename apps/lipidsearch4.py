...
# functions used in the main code of the app 
    ...
    def build_lipidsearch_df(lipid_search): # function that creates a pandas dataframe out of the uploaded LipidSearch dataset
        ...
        if counter_rej == 0:
            ...
            if ".txt" in lipid_search.name:
                ...
                if ('LipidMolec' and 'Class' and 'Calc Mass' and 'BaseRt') in df.columns.values.tolist():
 ...
 # function to calculate the abundance of sfa, mufa and pufa for each lipid species 
    ...
    def calculate_class_fold_change(X, cond_lst, rep_lst, control, experimental):
        ...
        group_temp = temp.groupby('Class')[["MainArea[s" + str(i+1) + ']' for i in range(total_reps)]].sum()
    ...
    def calculate_saturation_ratio(dataframe, class_lst):
        ...
        group_dataframe = dataframe.groupby('Class')[['total_sat', 'total_unsat']].sum()
        ...
        if view_abundance_chart: 
            ...
            for cond in cond_lst:
                ...
                if index == 0:
                    ...
                    group_temp["mean_AUC_" + cond] = group_temp[["MainArea[s" + str(i+1) + ']' for i in range(rep_lst_agg[0])]].apply(lambda x: np.mean(x), axis = 1)
                    
                    group_temp["std_AUC_" + cond] = group_temp[["MainArea[s" + str(i+1) + ']' for i in range(rep_lst_agg[0])]].apply(lambda x: np.std(x), axis = 1)
                    
                else:
                    
                    group_temp["mean_AUC_" + cond] = group_temp[["MainArea[s" + str(i+1) + "]" for i in range(rep_lst_agg[index - 1], rep_lst_agg[index])]]\
                                 .apply(lambda x: np.mean(x), axis = 1)
                                 
                    group_temp["std_AUC_" + cond] = group_temp[["MainArea[s" + str(i+1) + "]" for i in range(rep_lst_agg[index - 1], rep_lst_agg[index])]]\
    ...
    if lipid_search is not None:
                ...
                if df is not None:
                    ...
                    if confirm_data:
                        ...
                        expand_cleaned_data = st.expander("Remove Anomalies")
